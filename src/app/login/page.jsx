...
export default function LoginPage() {
  ...
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[var(--bg)] px-6 py-16">
      ...
      <div aria-hidden className="absolute inset-0" style={{ background: "linear-gradient(180deg, var(--bg) 0%, rgba(10,13,20,0.55) 40%, var(--bg) 100%)" }} />

      {/* glows decorativos */}
      <div aria-hidden className="pointer-events-none absolute left-1/2 top-0 h-[500px] w-[800px] -translate-x-1/2 -translate-y-1/2 rounded-full opacity-25 blur-[130px]" style={{ background: "linear-gradient(90deg, var(--blue), var(--purple))" }} />
      <div aria-hidden className="pointer-events-none absolute bottom-0 right-0 h-[300px] w-[300px] translate-x-1/3 translate-y-1/3 rounded-full opacity-20 blur-[100px]" style={{ background: "var(--purple)" }} />

      {/* voltar pro início */}
      <Link href="/" className="absolute left-6 top-6 z-10 flex items-center gap-1.5 text-sm font-medium transition-colors hover:text-[var(--text)]" style={{ color: "var(--muted)" }}>
      ...
      <div className="group relative z-10 w-full max-w-[440px]">
        ...
        <div className="relative rounded-3xl border px-9 py-11 shadow-2xl backdrop-blur-xl transition-transform duration-300 group-hover:-translate-y-1" style={{ backgroundColor: "rgba(16,20,29,0.72)", borderColor: "var(--border)" }}>
          
          {/* logo + badges */}
          <div className="mb-8 flex flex-col items-center gap-4 text-center">
            <LogoMark />
            <div>
              <span style={{ fontFamily: "'Space Grotesk', sans-serif", color: "var(--text)" }} className="text-2xl font-bold tracking-tight">
                <span style={{ color: "var(--blue)" }}>Dev</span>Road
              </span>
            </div>
            <div className="flex flex-wrap justify-center gap-2">
              {BADGES.map((badge) => (
                <span key={badge} style={{ borderColor: "var(--border)", color: "var(--muted)" }} className="rounded-full border px-3 py-1 text-[11px] font-medium">
                  {badge}
                </span>
              ))}
            </div>
